...
def _auth_required(min_role):
    ...
    def decorator(func):
        ...
        def _decorator(*args, **kwargs):
            ...
            if not config.allowlist_match(
                current_app.config.get('allow_login', []),
                request.remote_addr,
                request.host
            ):
...
def signOut():
    ...
    resp.delete_cookie(config.COOKIE_NAME, path='/', samesite='Lax')
    return resp
...
def refreshToken(token, request, minute=20):
    ...
    if now < (exp_time + timedelta(minutes=minute)):
        ...
        resp.set_cookie(config.COOKIE_NAME, value=new_token, **config.auth_cookie_settings(request))
        return resp
    return False
